...
import { RefreshCw, MousePointer2, Keyboard, Move, Key } from 'lucide-react';
...
export default function RemoteBrowser({ userId, adminToken }: RemoteBrowserProps) {
  ...
  const handleSubmit2FA = () => {
    if (twoFACode.length >= 4) {
      socket.emit('submit_2fa', { userId, code: twoFACode });
      setShow2FA(false);
      setTwoFACode('');
    }
  };
      ...
      <div className="bg-slate-800 p-3 flex items-center justify-between border-b border-slate-700">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-300">Remote Control</span>
          </div>
          <button 
